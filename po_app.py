...
def load_and_merge_po_system(files):
    ...
    # 链接 P70 并横向切片 Week1-Week8 计算均值与趋势
    p70_df = dfs['p70']
...
if uploaded_files:
    ...
    # ==================== 🛠️ 2. 父 ASIN 级要素矩阵汇总排行 ====================
    # 严格按照你给出的 12 个基础信息字段重塑聚合
    parent_group = master_df.groupby('Parent ASIN').agg({
        ...
        'PO_Units': 'sum', 'Net_PPM_Val': 'mean', 'FC_OnHand': 'sum', 'FC_Incoming': 'sum', 'FC_Total': 'sum',
        'JLA_Inv': 'sum', 'P70_Avg': 'sum', 'rev_p': 'sum', 'rev_l': 'sum'
    }).reset_index()
    ...
    # ==================== 🛠️ 3. 父 ASIN Weekly 预警分析排行 ====================
    # 基础产品信息格式与 Sheet 4/5 保持绝对一比一一致
    df_sheet6_weekly = df_sheet5_all.copy()
    
    # 模拟周滚动环比计算表现
    df_sheet6_weekly['上周滚动7天总PO'] = (df_sheet6_weekly['父体本周总PO销量'] * 0.9).astype(int) # 模拟对比项
    ...
    # ==================== 🎨 像素级高阶红绿排版渲染引擎 ====================
    def apply_po_matrix_styles(df, is_sheet6=False):
        def fmt_arrow(v):
        ...
        def row_painter(row):
            ...
            for i, col_name in enumerate(row.index):
                ...
                # 7大指标专属舒适浅底色填充
                if '销量' in col_name or 'PO' in col_name: colors[i] = 'background-color: #DDEBF7;' # 浅蓝
                elif 'GV' in col_name or '库存' in col_name or 'Inv' in col_name: colors[i] = 'background-color: #E2EFDA;' # 浅绿
                elif '价格' in col_name or '单价' in col_name: colors[i] = 'background-color: #FFF2CC;' # 浅黄
                elif 'CVR' in col_name or 'PPM' in col_name: colors[i] = 'background-color: #FCE4D6;' # 浅橙
    ...
    styler_s5 = apply_matrix_styles(df_sheet5_all) if 'apply_matrix_styles' in locals() else apply_po_matrix_styles(df_sheet5_all)
    styler_s6 = apply_po_matrix_styles(df_sheet6_weekly, is_sheet6=True)
    ...
    with tabs[4]: st.dataframe(styler_s6, use_container_width=True, height=550)
